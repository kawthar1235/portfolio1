...
export default function AdminDashboard() {
  ...
  const uploadCertImage = async () => {
    if (!certImageFile) return "";

    setCertUploading(true);
    const data = new FormData();
    data.append("file", certImageFile);
    data.append("upload_preset", UPLOAD_PRESET);
    ...
    try {
      ...
      setCertUploading(false);
      showToast("❌ Certificate image upload failed");
      return "";
    }
  };
  ...
  const addCertificate = async (e) => {
    ...
    if (!imageUrl) {
      showToast("❌ Certificate image is required");
      return;
    }

    try {
      const created = await api("/certificates", "POST", {
        ...certForm,
        image: imageUrl,
      });
  ...
  const deleteCertificate = async (id) => {
    ...
    try {
      ...
      showToast("🗑️ Certificate deleted!");
    } catch {
      showToast("❌ Failed to delete");
    }
  };
        ...
        {view === "certificates" && (
          <div style={s.msgGrid}>
            <div style={s.msgCard}>
              ...
              <form onSubmit={addCertificate} style={s.formGrid}>
                <div style={s.formGroup}>
                ...
                <div style={s.formGroup}>
                  <label style={s.label}>Title (Arabic)</label>
                  <input
                    style={s.input}
                    placeholder="شهادة تجربة المستخدم"
                    value={certForm.titleAr}
                    onChange={(e) =>
                      setCertForm((f) => ({ ...f, titleAr: e.target.value }))
                    }
                  />
                </div>
                ...
                <div style={s.formGroup}>
                  <label style={s.label}>Issuer (Arabic)</label>
                  <input
                    style={s.input}
                    placeholder="جوجل"
                    value={certForm.issuerAr}
                    onChange={(e) =>
                      setCertForm((f) => ({ ...f, issuerAr: e.target.value }))
                    }
                  />
                </div>
                ...
                <div style={s.formGroup}>
                  ...
                  <input
                    ...
                    value={certForm.link}
                    onChange={(e) =>
                      setCertForm((f) => ({ ...f, link: e.target.value }))
                    }
                  />
                </div>
                  ...
                  <div style={s.imageUploadWrap}>
                    ...
                    <label style={s.uploadLabel}>
                      <input
                        ...
                        style={{ display: "none" }}
                      />
                      {certImagePreview ? "🔄 Change Image" : "📷 Upload Image"}
                    </label>
                ...
                >
                  ...
                  />
                  <label htmlFor="certFeatured" style={{ ...s.label, margin: 0 }}>
                    Mark as Featured
                  </label>
                </div>

                <div style={{ gridColumn: "1/-1" }}>
                  <button type="submit" style={s.btnPrimary} disabled={certUploading}>
            ...
            {certificates.length === 0 && (
              <div style={s.empty}>No certificates yet — add them from here!</div>
            )}
                  ...
                  <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
                    ...
                    <div>
                      <div style={s.msgName}>
                        {cert.title}
                        {cert.featured && (
                          <span style={{ marginLeft: "0.5rem", color: "#F2A4A5" }}>
                            ★
                          </span>
                        )}
                      </div>
