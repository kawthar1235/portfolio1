...
const EMPTY_PROJECT = {
  ...
  techStack: "", liveUrl: "", githubUrl: "", year: new Date().getFullYear().toString(),
  featured: false,
};
...
export default function AdminDashboard() {
  ...
  // ── PROJECTS CRUD ──
  const openAdd  = () => { setForm(EMPTY_PROJECT); setEditId(null); setModal("add"); };
  const openEdit = (p) => {
  ...
  if (!token) return (
    <div style={s.loginWrap}>
      <div style={s.loginCard}>
        <div style={s.loginLogo}>nour<span style={{color:"#F2A4A5",fontStyle:"italic"}}>.</span></div>
        <div style={s.loginTitle}>Admin Panel</div>
        <form onSubmit={handleLogin} style={s.loginForm}>
          <input style={s.input} placeholder="Username" value={login.username}
            onChange={e => setLogin(l => ({...l, username: e.target.value}))} required />
          <input style={s.input} type="password" placeholder="Password" value={login.password}
            onChange={e => setLogin(l => ({...l, password: e.target.value}))} required />
  ...
  return (
    <div style={s.wrap}>
      ...
      <aside style={s.sidebar}>
        <div style={s.sidebarLogo}>nour<span style={{color:"#F2A4A5",fontStyle:"italic"}}>.</span></div>
        <nav style={s.nav}>
          {[["projects","◈ Projects"],["messages","✉ Messages"]].map(([v,l]) => (
      ...
      <main style={s.main}>
        ...
        <div style={s.header}>
          <div>
            <div style={s.headerTitle}>{view === "projects" ? "Projects" : "Messages"}</div>
            <div style={s.headerSub}>
              {view === "projects" ? `${projects.length} total` : `${messages.length} messages`}
            </div>
          </div>
          {view === "projects" && (
          ...
          <>
            ...
            <div style={s.tabs}>
              {[["all","All"],["code","Code"],["design","Design"]].map(([v,l]) => (
                <button key={v} style={{...s.tab, ...(filter===v ? s.tabActive : {})}}
            ...
            <div style={s.table}>
              ...
              {filtered.length === 0 && (
                <div style={s.empty}>No projects yet — click "+ Add Project" to start!</div>
              )}
              {filtered.map(p => (
                <div key={p._id} style={s.tableRow}>
                  <span style={{flex:2, fontWeight:500, color:"#e8e4ff"}}>{p.title}</span>
                  <span style={{flex:1}}>
                    <span style={{...s.badge, background: p.type==="code" ? "rgba(91,163,204,.2)" : "rgba(242,164,165,.2)",
                  ...
                  <span style={{flex:1}}>
                    {p.featured && <span style={{...s.badge, background:"rgba(242,164,165,.15)", color:"#F2A4A5"}}>★ Featured</span>}
                  </span>
          ...
          <div style={s.modalCard} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={saveProject} style={s.formGrid}>
              ...
              <div style={{...s.formGroup, gridColumn:"1/-1"}}>
                ...
                <textarea style={{...s.input, height:"80px", resize:"none"}} value={form.description}
              ...
              <div style={{...s.formGroup, gridColumn:"1/-1"}}>
                ...
                <input style={s.input} placeholder="React, Node.js, MongoDB" value={form.techStack}
                  onChange={e => setForm(f=>({...f,techStack:e.target.value}))} />
              </div>
              <div style={s.formGroup}>
                <label style={s.label}>Live URL</label>
                <input style={s.input} placeholder="https://..." value={form.liveUrl}
                  onChange={e => setForm(f=>({...f,liveUrl:e.target.value}))} />
              </div>
              <div style={s.formGroup}>
                <label style={s.label}>GitHub URL</label>
                <input style={s.input} placeholder="https://github.com/..." value={form.githubUrl}
                  onChange={e => setForm(f=>({...f,githubUrl:e.target.value}))} />
              </div>
              <div style={{...s.formGroup, gridColumn:"1/-1", flexDirection:"row", alignItems:"center", gap:"0.75rem"}}>
                <input type="checkbox" id="featured" checked={form.featured}
                  onChange={e => setForm(f=>({...f,featured:e.target.checked}))} style={{width:"18px",height:"18px"}} />
              ...
              <div style={{gridColumn:"1/-1", display:"flex", gap:"0.75rem", justifyContent:"flex-end", marginTop:"0.5rem"}}>
                ...
                <button type="submit" style={s.btnPrimary} disabled={loading}>
                  {loading ? "Saving…" : modal === "add" ? "Add Project" : "Save Changes"}
                </button>
              </div>
            </form>
          </div>
...
// ── STYLES ────────────────────────────────────────────────────────
const s = {
  wrap:        { display:"flex", minHeight:"100vh", background:"#0b0b1a", fontFamily:"'DM Sans',sans-serif", color:"rgba(232,228,255,.7)" },
  ...
  tableRow:    { display:"flex", alignItems:"center", padding:"1rem 1.5rem", borderBottom:"1px solid rgba(232,228,255,.05)", transition:"background 0.2s", fontSize:"0.85rem" },
  ...
  overlay:     { position:"fixed", inset:0, background:"rgba(0,0,0,.6)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999 },
  modalCard:   { background:"#111128", border:"1px solid rgba(232,228,255,.1)", borderRadius:"1.2rem", padding:"2rem", width:"600px", maxWidth:"95vw", maxHeight:"90vh", overflowY:"auto" },
  ...
  btnPrimary:  { background:"#5BA3CC", color:"#fff", border:"none", padding:"0.65rem 1.5rem", borderRadius:"2rem", fontFamily:"'DM Sans',sans-serif", fontSize:"0.8rem", cursor:"pointer", transition:"all 0.2s" },
  ...
  loginWrap:   { minHeight:"100vh", background:"#0b0b1a", display:"flex", alignItems:"center", justifyContent:"center" },
