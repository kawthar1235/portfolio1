...
export default function AdminDashboard() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setToken(data.token);
    } catch (err) { setLoginErr(err.message || "Invalid credentials"); }
  };

  const handleLogout = () => { localStorage.removeItem("adminToken"); setToken(""); };
  ...
  const openEdit = (p) => { setForm({ ...p, techStack: Array.isArray(p.techStack) ? p.techStack.join(", ") : p.techStack || "" }); setEditId(p._id); setImageFile(null); setImagePreview(p.image || ""); setModal("edit"); };
  ...
  const saveProject = async (e) => {
    ...
    try {
      ...
      else { const u = await api(`/projects/${editId}`,"PUT",payload); setProjects(ps=>ps.map(p=>p._id===editId?u:p)); showToast("✅ Project updated!"); }
      setModal(null);
  ...
  const deleteProject = async (id) => {
    ...
    try { await api(`/projects/${id}`,"DELETE"); setProjects(ps=>ps.filter(p=>p._id!==id)); showToast("🗑️ Project deleted!"); }
    catch { showToast("❌ Failed to delete"); }
  };

  const deleteMessage = async (id) => {
    if (!confirm("Delete this message?")) return;
    try { await api(`/admin/messages/${id}`,"DELETE"); setMessages(ms=>ms.filter(m=>m._id!==id)); showToast("🗑️ Message deleted!"); }
    catch { showToast("❌ Failed to delete"); }
  };

  const addSkill = async (e) => {
    e.preventDefault();
    try { const c = await api("/skills","POST",skillForm); setSkills(prev=>[c,...prev]); setSkillForm(EMPTY_SKILL); showToast("✅ Skill added!"); }
    catch (err) { showToast("❌ "+(err.message||"Error")); }
  };

  const deleteSkill = async (id) => {
    if (!confirm("Delete this skill?")) return;
    try { await api(`/skills/${id}`,"DELETE"); setSkills(prev=>prev.filter(s=>s._id!==id)); showToast("🗑️ Skill deleted!"); }
    catch { showToast("❌ Failed to delete"); }
  };
  ...
  if (!token) return (
    <div style={{...s.loginWrap,...ADMIN_STYLE}} className="admin-page">
      <div style={s.loginCard}>
        <div style={s.loginLogo}>kawthar<span style={{color:"#F2A4A5",fontStyle:"italic"}}>.</span></div>
        <div style={s.loginTitle}>Admin Panel</div>
        <form onSubmit={handleLogin} style={s.loginForm}>
          <input style={s.input} placeholder="Username" value={login.username} onChange={e=>setLogin(l=>({...l,username:e.target.value}))} required />
          <input style={s.input} type="password" placeholder="Password" value={login.password} onChange={e=>setLogin(l=>({...l,password:e.target.value}))} required />
          {loginErr && <div style={s.err}>{loginErr}</div>}
          <button style={s.btnPrimary} type="submit">Sign In</button>
        </form>
      </div>
    </div>
  );
    ...
    <div style={{...s.wrap,...ADMIN_STYLE}} className="admin-page">
      ...
      <main style={s.main}>
        ...
        {view==="projects" && (
          <>
            <div style={s.tabs}>
              {[["all","All"],["code","Code"],["design","Design"]].map(([v,l])=>(
            ...
            <div style={s.table}>
              ...
              {filtered.map(p=>(
                <div key={p._id} style={s.tableRow}>
                  ...
                  <span style={{flex:2,fontWeight:500,color:"#e8e4ff"}}>{p.title}</span>
                  <span style={{flex:1}}><span style={{...s.badge,background:p.type==="code"?"rgba(91,163,204,.2)":"rgba(242,164,165,.2)",color:p.type==="code"?"#7ec8e8":"#F2A4A5"}}>{p.type}</span></span>
                  <span style={{flex:1,color:"rgba(232,228,255,.5)",fontSize:"0.8rem"}}>{p.category}</span>
                  <span style={{flex:1,color:"rgba(232,228,255,.5)",fontSize:"0.8rem"}}>{p.year}</span>
                  <span style={{flex:1}}>{p.featured&&<span style={{...s.badge,background:"rgba(242,164,165,.15)",color:"#F2A4A5"}}>★</span>}</span>
                  <span style={{flex:1,display:"flex",gap:"0.5rem"}}>
                    <button style={s.btnEdit} onClick={()=>openEdit(p)}>Edit</button>
                    <button style={s.btnDelete} onClick={()=>deleteProject(p._id)}>Del</button>
                  </span>
                </div>
              ))}
            </div>
          ...
          <div style={s.msgGrid}>
            ...
            {messages.map(m=>(
              <div key={m._id} style={s.msgCard}>
                <div style={s.msgTop}>
                  <div><div style={s.msgName}>{m.name}</div><div style={s.msgEmail}>{m.email}</div></div>
                  <button style={s.btnDelete} onClick={()=>deleteMessage(m._id)}>Delete</button>
                </div>
                {m.subject && <div style={s.msgSubject}>{m.subject}</div>}
                <div style={s.msgBody}>{m.message}</div>
                <div style={s.msgDate}>{new Date(m.createdAt).toLocaleDateString()}</div>
              </div>
            ))}
          </div>
        )}
            ...
            <div style={s.msgCard}>
              ...
              <form onSubmit={addSkill} style={s.formGrid}>
                <div style={s.formGroup}>
                  ...
                  <select style={s.input} value={skillForm.icon} onChange={e=>setSkillForm(f=>({...f,icon:e.target.value}))} required>
                    <option value="">Choose icon</option>
                    {SKILL_ICONS.map(icon=><option key={icon} value={icon}>{icon}</option>)}
                  </select>
                </div>
                <div style={s.formGroup}>
                  <label style={s.label}>Name *</label>
                  <input style={s.input} placeholder="Visual Identity" value={skillForm.name} onChange={e=>setSkillForm(f=>({...f,name:e.target.value}))} required />
                </div>
                <div style={{...s.formGroup,gridColumn:"1/-1"}}>
                  <label style={s.label}>List *</label>
                  <input style={s.input} placeholder="Logo design · Color systems" value={skillForm.list} onChange={e=>setSkillForm(f=>({...f,list:e.target.value}))} required />
                </div>
                <div style={{gridColumn:"1/-1"}}><button type="submit" style={s.btnPrimary}>+ Add Skill</button></div>
              </form>
            </div>
            {skills.length===0 && <div style={s.empty}>No skills yet!</div>}
            {skills.map(skill=>(
              <div key={skill._id} style={s.msgCard}>
                <div style={s.msgTop}>
                  <div><div style={s.msgName}>{skill.icon} {skill.name}</div><div style={s.msgBody}>{skill.list}</div></div>
                  <button style={s.btnDelete} onClick={()=>deleteSkill(skill._id)}>Delete</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {modal && (
        <div style={s.overlay} onClick={()=>setModal(null)}>
          <div style={s.modalCard} onClick={e=>e.stopPropagation()}>
            <div style={s.modalTitle}>{modal==="add"?"Add New Project":"Edit Project"}</div>
            <form onSubmit={saveProject} style={s.formGrid}>
              <div style={s.formGroup}><label style={s.label}>Title *</label><input style={s.input} value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} required /></div>
              <div style={s.formGroup}><label style={s.label}>Year</label><input style={s.input} value={form.year} onChange={e=>setForm(f=>({...f,year:e.target.value}))} /></div>
              <div style={{...s.formGroup,gridColumn:"1/-1"}}><label style={s.label}>Description *</label><textarea style={{...s.input,height:"80px",resize:"none"}} value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} required /></div>
              <div style={s.formGroup}>
                <label style={s.label}>Type *</label>
                <select style={s.input} value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value,category:e.target.value==="code"?"web":"Branding"}))}>
                  <option value="code">Code</option><option value="design">Design</option>
                </select>
              </div>
              <div style={s.formGroup}>
                <label style={s.label}>Category *</label>
                <select style={s.input} value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))}>
                  {form.type==="code"?["web","app","tool"].map(c=><option key={c} value={c}>{c}</option>):["Branding","UI","Illustration"].map(c=><option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div style={{...s.formGroup,gridColumn:"1/-1"}}><label style={s.label}>Tech Stack (comma separated)</label><input style={s.input} placeholder="React, Node.js, MongoDB" value={form.techStack} onChange={e=>setForm(f=>({...f,techStack:e.target.value}))} /></div>
              <div style={s.formGroup}><label style={s.label}>Live URL</label><input style={s.input} placeholder="https://..." value={form.liveUrl} onChange={e=>setForm(f=>({...f,liveUrl:e.target.value}))} /></div>
              <div style={s.formGroup}><label style={s.label}>GitHub URL</label><input style={s.input} placeholder="https://github.com/..." value={form.githubUrl} onChange={e=>setForm(f=>({...f,githubUrl:e.target.value}))} /></div>
              <div style={{...s.formGroup,gridColumn:"1/-1"}}>
                <label style={s.label}>Project Image</label>
                <div style={s.imageUploadWrap}>
                  {imagePreview && (<div style={s.imagePreviewWrap}><img src={imagePreview} alt="preview" style={s.imagePreview} /><button type="button" style={s.removeImg} onClick={()=>{setImageFile(null);setImagePreview("");setForm(f=>({...f,image:""}));}}>✕ Remove</button></div>)}
                  <label style={s.uploadLabel}><input type="file" accept="image/*" onChange={handleImageChange} style={{display:"none"}} />{imagePreview?"🔄 Change Image":"📷 Upload Image"}</label>
                  <div style={s.uploadHint}>JPG, PNG, WEBP — max 10MB</div>
                </div>
              </div>
              <div style={{...s.formGroup,gridColumn:"1/-1",flexDirection:"row",alignItems:"center",gap:"0.75rem"}}>
                <input type="checkbox" id="featured" checked={form.featured} onChange={e=>setForm(f=>({...f,featured:e.target.checked}))} style={{width:"18px",height:"18px"}} />
                <label htmlFor="featured" style={{...s.label,margin:0}}>Mark as Featured</label>
              </div>
              <div style={{gridColumn:"1/-1",display:"flex",gap:"0.75rem",justifyContent:"flex-end",marginTop:"0.5rem"}}>
                <button type="button" style={s.btnGhost} onClick={()=>setModal(null)}>Cancel</button>
                <button type="submit" style={s.btnPrimary} disabled={loading||uploading}>{uploading?"Uploading...":loading?"Saving...":modal==="add"?"Add Project":"Save Changes"}</button>
              </div>
            </form>
          </div>
        </div>
