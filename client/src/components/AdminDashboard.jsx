...
const EMPTY_PROJECT = {
  title: "", description: "", type: "code", category: "web",
  techStack: "", liveUrl: "", githubUrl: "",
  year: new Date().getFullYear().toString(), featured: false, image: "",
};
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (!token) return;
    api("/projects").then((data) => setProjects(Array.isArray(data) ? data : [])).catch(console.error);
    api("/admin/messages").then((data) => setMessages(Array.isArray(data) ? data : [])).catch(console.error);
    api("/skills").then((data) => setSkills(Array.isArray(data) ? data : [])).catch(console.error);
  }, [token]);
  ...
  const openEdit = (p) => {
    ...
    setEditId(p._id); setImageFile(null); setImagePreview(p.image || ""); setModal("edit");
  };
  ...
  const addSkill = async (e) => {
    e.preventDefault();
    try { const created = await api("/skills", "POST", skillForm); setSkills((prev) => [created, ...prev]); setSkillForm(EMPTY_SKILL); showToast("✅ Skill added!"); }
    catch (err) { showToast("❌ " + (err.message || "Error")); }
  };
            ...
            <div style={s.table}>
              ...
              {filtered.map((p) => (
                <div key={p._id} style={s.tableRow}>
                  <span style={{ flex: 0.5 }}>
                    {p.image ? <img src={p.image} alt={p.title} style={s.thumb} /> : <div style={s.thumbEmpty}>—</div>}
                  </span>
                  <span style={{ flex: 2, fontWeight: 500, color: "#e8e4ff" }}>{p.title}</span>
                  <span style={{ flex: 1 }}>
                    <span style={{ ...s.badge, background: p.type === "code" ? "rgba(91,163,204,.2)" : "rgba(242,164,165,.2)", color: p.type === "code" ? "#7ec8e8" : "#F2A4A5" }}>{p.type}</span>
                  </span>
            ...
            <div style={s.msgCard}>
              ...
              <form onSubmit={addSkill} style={s.formGrid}>
                ...
                <div style={{ ...s.formGroup, gridColumn: "1/-1" }}>
                  ...
                  <input style={s.input} placeholder="Logo design · Color systems · Typography" value={skillForm.list} onChange={(e) => setSkillForm((f) => ({ ...f, list: e.target.value }))} required />
                </div>
                <div style={{ gridColumn: "1/-1" }}>
                  <button type="submit" style={s.btnPrimary}>+ Add Skill</button>
                </div>
              </form>
            </div>
            {skills.length === 0 && <div style={s.empty}>No skills yet — add them from here!</div>}
            {skills.map((skill) => (
              <div key={skill._id} style={s.msgCard}>
                <div style={s.msgTop}>
                  <div>
                    <div style={s.msgName}>{skill.icon} {skill.name}</div>
          ...
          <div style={s.modalCard} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={saveProject} style={s.formGrid}>
              ...
              <div style={s.formGroup}>
                ...
                <input style={s.input} value={form.year} onChange={(e) => setForm((f) => ({ ...f, year: e.target.value }))} />
              </div>
              <div style={{ ...s.formGroup, gridColumn: "1/-1" }}>
                <label style={s.label}>Description *</label>
                <textarea style={{ ...s.input, height: "80px", resize: "none" }} value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} required />
              </div>
              <div style={s.formGroup}>
              ...
              <div style={s.formGroup}>
                ...
                <select style={s.input} value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))}>
                  {form.type === "code"
                    ? ["web", "app", "tool"].map((c) => <option key={c} value={c}>{c}</option>)
                    : ["Branding", "UI", "Illustration"].map((c) => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div style={{ ...s.formGroup, gridColumn: "1/-1" }}>
                <label style={s.label}>Tech Stack (comma separated)</label>
                <input style={s.input} placeholder="React, Node.js, MongoDB" value={form.techStack} onChange={(e) => setForm((f) => ({ ...f, techStack: e.target.value }))} />
              </div>
              <div style={s.formGroup}>
                <label style={s.label}>Live URL</label>
                <input style={s.input} placeholder="https://..." value={form.liveUrl} onChange={(e) => setForm((f) => ({ ...f, liveUrl: e.target.value }))} />
              </div>
              <div style={s.formGroup}>
                <label style={s.label}>GitHub URL</label>
                <input style={s.input} placeholder="https://github.com/..." value={form.githubUrl} onChange={(e) => setForm((f) => ({ ...f, githubUrl: e.target.value }))} />
              </div>
              <div style={{ ...s.formGroup, gridColumn: "1/-1" }}>
                ...
                <div style={s.imageUploadWrap}>
                  {imagePreview && (
                    <div style={s.imagePreviewWrap}>
                      ...
                      <button type="button" style={s.removeImg} onClick={() => { setImageFile(null); setImagePreview(""); setForm((f) => ({ ...f, image: "" })); }}>✕ Remove</button>
                    </div>
                  )}
              ...
              <div style={{ gridColumn: "1/-1", display: "flex", gap: "0.75rem", justifyContent: "flex-end", marginTop: "0.5rem" }}>
                ...
                <button type="submit" style={s.btnPrimary} disabled={loading || uploading}>
                  {uploading ? "Uploading image…" : loading ? "Saving…" : modal === "add" ? "Add Project" : "Save Changes"}
                </button>
              </div>
            </form>
          </div>
